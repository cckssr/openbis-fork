...
public class ImagingService implements ICustomASServiceExecutor
{
    ...
    {
        return executeService(context.getSessionToken(), "imaging", options.getParameters());
    }
    ...
    private File getRootFile(DataSet dataSet) {
        return getRootFile(dataSet, true);
    }

    private File getRootFile(DataSet dataSet, boolean isDss)
    {
        Path path;
        if(isDss) {
            path  = Path.of(this.storeRootDss, dataSet.getPhysicalData().getShareId(), dataSet.getPhysicalData().getLocation());
        } else {
    ...
    private File getRootFile(String sessionToken, Sample sample) throws IOException
    {
        final ISessionWorkspaceProvider sessionWorkspaceProvider = CommonServiceProvider.getSessionWorkspaceProvider();
        return sessionWorkspaceProvider.getSessionWorkspace(sessionToken).getCanonicalFile();
    }

    private ImagingDataSetPropertyConfig processUserDefinedImagingData(File rootFile, IPropertiesHolder entity)
    {
        List<Map.Entry<File, String>> imageFiles = detectImages(rootFile);
        ...
        for (int i = 0; i < imageFiles.size(); i++)
        {
            ...
            metaData.put("static image", "true");
            metaData.put("filename", imageFile.getKey().getName());
        ...
        ImagingDataSetPropertyConfig config =
                Util.readConfig(entity.getJsonProperty(IMAGING_CONFIG_PROPERTY_NAME),
    ...
    private ImagingPreviewContainer processPreviewFlow(String sessionToken,
            ImagingPreviewContainer data) throws IOException
    {
        ...
        if (entity instanceof DataSet dataSet)
        {
            ...
            {
                File rootFile = getRootFile(dataSet, true);
                ImagingDataSetPropertyConfig config = processUserDefinedImagingData(rootFile, dataSet);
                ...
                update.getMetaData().put(PREVIEW_TOTAL_COUNT, Integer.toString(config.getImages().get(0).getPreviews().size()));
        ...
        } else
        {
            ...
            if (entity instanceof DataSet dataSet)
            {
                ...
                if(adaptor == null || adaptor.isBlank()) {
                    ...
                    DataSetUpdate update = new DataSetUpdate();
                    update.setDataSetId(dataSet.getPermId());
                    update.setProperty(IMAGING_CONFIG_PROPERTY_NAME, Util.convertConfigToJson(dataSetConfig));
                    update.getMetaData().put(PREVIEW_TOTAL_COUNT, Integer.toString(dataSetConfig.getImages().get(0).getPreviews().size()));
                    getApplicationServerApi().updateDataSets(sessionToken, Arrays.asList(update));
                    return data;
                }
            } else if(entity instanceof Sample sample) {
                Map<IDataSetId, DataSet> map = getDataSets(sessionToken, List.of(sample.getPermId().getPermId()));
                if(map.isEmpty())
                {
                ...
                if(!computedRoot.exists()) {
                    ...
                    afs.downloadEntityFiles(sample.getPermId().getPermId(), sessionWorkspaceDirectory.toPath());
                ...
                String adaptor = image.getConfig().getAdaptor();
                if(adaptor == null || adaptor.isBlank()) {
                    ImagingDataSetPropertyConfig sampleConfig = processUserDefinedImagingData(rootFile, sample);

                    SampleUpdate update = new SampleUpdate();
                    update.setSampleId(sample.getPermId());
                    update.setProperty(IMAGING_CONFIG_PROPERTY_NAME, Util.convertConfigToJson(sampleConfig));
                    update.getMetaData().put(PREVIEW_TOTAL_COUNT, Integer.toString(sampleConfig.getImages().get(0).getPreviews().size()));
                    getApplicationServerApi().updateSamples(sessionToken, Arrays.asList(update));
                    return data;
                }
    ...
    private List<Map.Entry<File, String>> detectImages(File rootFile) {
    ...
    {
        // Get all parameters
        IPropertiesHolder propertiesHolder = getData(sessionToken, data.getPermId());
        final DataSet dataSet = getDataSet(sessionToken, data.getPermId());
        final ImagingDataSetPropertyConfig config =
                Util.readConfig(propertiesHolder.getJsonProperty(IMAGING_CONFIG_PROPERTY_NAME),
                        ImagingDataSetPropertyConfig.class);
        File rootFile;
        if(propertiesHolder instanceof Sample) {
        ...
        if(!dataSet.getType().getCode().equals(USER_DEFINED_IMAGING_DATA_TYPE)
            && !dataSet.getDataStore().getCode().contains("AFS"))
        {
        ...
        for (ImagingExportIncludeOptions exportType : exportConfig.getInclude())
        {
            ...
            {
                archiveRawData(rootFile, "", archiver, propertiesHolder.getProperties(),
                        data.getPermId());
            } else
            {
            ...
            ImagingMultiExportContainer data)
    {
        ...
        for (String permId : sortedExports.keySet())
        {
            ...
            IPropertiesHolder propertiesHolder;
            if (dataSet == null)
            {
                throw new UserFailureException("Could not find data set: " + permId);
            }
            if(dataSet.getDataStore().getCode().contains("AFS")) {
                propertiesHolder = getData(sessionToken, permId);
            } else {
                propertiesHolder = dataSet;
            }
            ...
            for (ImagingDataSetMultiExport export : sortedExports.get(permId))
            {
                ...
                for (ImagingExportIncludeOptions exportType : exportConfig.getInclude())
                {
                    ...
                    {
                        archiveRawData(rootFile, export.getPermId(), archiver, propertiesHolder.getProperties(), permId);
                    } else
                    {
                        throw new UserFailureException("Unknown export type!");
                    }
    ...
    private void archiveRawData(File rootFile, String rootFolderName,
            ImagingArchiver archiver, Map<String, Serializable> properties, String permId)
    {
        ...
        if (!properties.isEmpty())
        {
            ...
            archiver.addToArchive(rootFolderName,
                    String.format("%s_properties.txt", permId), json);
        }
    }
    ...
    {
        return CommonServiceProvider.getDataStoreServerApi();
    }
    ...
    private IPropertiesHolder getData(String sessionToken, String permId)
    {
        if(afs.isSessionValid())
        {
            ...
            Map<ISampleId, Sample> sampleResult = getApplicationServerApi()
                    .getSamples(sessionToken, Arrays.asList(samplePermId),
                            sampleFetchOptions);
        ...
        if (result.isEmpty())
        {
            throw new UserFailureException(String.format("Could not find Entity: '%s'", permId));
        }
        return result.get(dataSetPermId);
    }
