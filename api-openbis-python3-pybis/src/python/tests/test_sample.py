...
def test_create_samples_pointing_to_samples_in_transaction(space):
    o = space.openbis
    space_code = space.code

    timestamp = time.strftime("%a_%y%m%d_%H%M%S").lower()
    sample_code = 'MY_TYPE-' + timestamp
    new_st = o.new_sample_type(
        code=sample_code,
        generatedCodePrefix="TYPE",
        autoGeneratedCode=True,
        subcodeUnique=False,
        listable=True,
        showContainer=False,
    ...
    property_code = 'object_property-' + timestamp
    new_pt = o.new_property_type(
        code=property_code,
        label='object_property',
        description='single value object property for test',
        dataType='SAMPLE',

    )
    new_pt.save()
    new_st = o.get_sample_type(sample_code)
    new_st.assign_property('name')
    new_st.assign_property(new_pt)


    code = o.gen_code('SAMPLE', sample_code)
    s1 = o.new_sample(code=code, type=sample_code, space=space,
                      props={'name': 'TRANSACTION_CODE_1', property_code: f'/{space_code}/{code}'})
    s2 = o.new_sample(type=sample_code, space=space,
                      props={'name': 'TRANSACTION_CODE_2', property_code:  f'/{space_code}/{code}'})
    ...
    sample1 = o.get_sample(s1.permId)
    assert sample1 is not None
    assert sample1.props['name'] == 'TRANSACTION_CODE_1'
    assert sample1.props[property_code] == sample1.permId

    sample2 = o.get_sample(s2.permId)
    assert sample2 is not None
    assert sample2.props['name'] == 'TRANSACTION_CODE_2'
    assert sample2.props[property_code] == sample1.permId
