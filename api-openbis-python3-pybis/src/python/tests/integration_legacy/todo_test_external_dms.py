...
def create_external_data_management_system(openbis_instance):
    ...
    result = openbis_instance.create_external_data_management_system(
        code, "Test git", "localhost:~openbis/repo"
    )
    return code, result
...
def test_new_git_data_set(openbis_instance):
    ...
    result = openbis_instance.new_git_data_set(
        "GIT_REPO", "./", "12345", dms_code, "/DEFAULT/DEFAULT"
    )
    assert result is not None
...
def test_new_git_data_set_with_parent(openbis_instance):
    ...
    result = openbis_instance.new_git_data_set(
        "GIT_REPO", "./", "23456", dms_code, "/DEFAULT/DEFAULT", parents=parent_code
    )
