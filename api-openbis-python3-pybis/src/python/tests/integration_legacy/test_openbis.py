...
def test_get_samples_update_in_transaction(openbis_instance):
    ...
    st = openbis_instance.new_sample_type(
        "TEST_YEAST",
        "YEAST-",
    ...
    experiment = openbis_instance.new_experiment(
        code="MY_NEW_EXPERIMENT", type="DEFAULT_EXPERIMENT", project=project.code
    )
    experiment.save()
    ...
    sample1 = openbis_instance.get_sample(
        sample_ident=sample1.identifier, space=space.code, props="*"
    )
    sample2 = openbis_instance.get_sample(
        sample_ident=sample2.identifier, space=space.code, props="*"
    )
    ...
    samples = openbis_instance.get_samples(
        space=space.code, props="*", withChildren="*"
    )
    for sample in samples:
    ...
    samples = openbis_instance.get_samples(
        space=space.code, identifier=sample1.identifier
    )
    for sample in samples:
    ...
    samples = openbis_instance.get_samples(
        space=space.code, identifier=sample2.identifier
    )
    for sample in samples:
    ...
    sample3 = openbis_instance.get_sample(
        sample_ident=sample3.identifier, space=space.code, props="*"
    )
