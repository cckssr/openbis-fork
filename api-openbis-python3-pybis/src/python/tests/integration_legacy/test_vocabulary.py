...
def test_create_set_and_remove_vocabulary_property(space):
    ...
    property_type_code = (
        "test_vocabulary_property_type_" + timestamp + "_" + str(uuid.uuid4())
    )
    ...
    sample = space.new_sample(
        ...
        props={property_type_code.lower(): "term_code1"},
    )
    sample.save()
...
def test_create_delete_vocabulary_term(openbis_instance):
    ...
    voc_term = o.new_term(
        "term_code4", voc_code.upper(), "term_label4", "term_description4"
    )
    voc_term.save()
