...
def get_download_response(
    sequence_number, perm_id, file, is_directory, offset, payload
):
...
def start_download_session(ranges, wished_number_of_streams):
    return (
        ...
        "streamIds": ["""
        + ", ".join([f'"s{x}"' for x in range(int(wished_number_of_streams))])
        + """] }"""
    )
...
def run_around_tests(base_data):
    ...
    ranges = {"1/original/test_file.txt": "0:9"}
    MyServer.next_response = {
        "queue": "",
        "finishDownloadSession": "",
        "counter": 0,
        "parts": 10,
        "createFastDownloadSession": create_fast_download_session(
            perm_id, file, download_url, streams
        ),
...
def test_download_file(run_around_tests):
    ...
    downloaded_files = [
        os.path.join(dp, f) for dp, dn, fn in os.walk(temp_folder) for f in fn
    ]
    assert len(downloaded_files) == 1
    assert downloaded_files[0].endswith(file)
    import functools

    expected_outcome = functools.reduce(
        lambda a, b: a + b, [bytearray([x] * 10) for x in range(10)]
    )
