...
def executeCommand(
    ...
    try:
        ...
        p = subprocess.Popen(
            commandWithArguments, stdin=processIn, stdout=processOut, encoding="utf8"
        )
...
def unzip(zipFile, destination):
    ...
    executeCommand(
        ...
        "Couldn't unzip %s at %s" % (zipFile, destination),
    )
...
def copyFromTo(sourceFolder, destinationFolder, relativePathInSourceFolder):
    ...
    printAndFlush(
        ...
        % (relativePathInSourceFolder, sourceFolder, destinationFolder)
    )
...
def createDatabase(psqlExe, database, scriptPath=None):
    ...
    executeCommand(
        ...
        suppressStdOut=True,
        failingMessage="Couldn't execute script %s for database %s"
        % (scriptPath, database),
    )
...
def queryDatabase(psqlExe, database, queryStatement, showHeaders=False):
    ...
    lines = executeCommand(
        ...
        "Couldn't execute query: %s" % queryStatement,
        suppressStdOut=True,
    )
    result = []
    for line in lines:
...
def renderDuration(duration):
    ...
    if duration > 80:
        ...
        seconds = duration % 60
        if seconds > 0:
            renderedDuration = "%s and %s" % (
                renderNumber(minutes, "minute"),
                renderNumber(seconds, "second"),
            )
        else:
...
class LogMonitor:
    ...
    def waitUntilEvent(self, condition, startTime=None, delay=0):
        ...
        self.conditions.append(condition)
        renderedStartTime = time.strftime(
            ...
            while True:
                ...
                while True:
                    ...
                    if actualTime > finalTime:
                        self.printer.printMsg(
                            f"Time out detected! start time: {renderedStartTime}, calculated end time: {self.getFormattedTime(finalTime)}, current time: {self.getFormattedTime(actualTime)}, timeout (min): {self.timeOutInMinutes}"
                        )
                        raise Exception(
                            "Time out after %d minutes for monitoring %s log."
                    ...
                    if line == "":
                        break
                    match = re.match(
                        "(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}),(\d{3}) (.{6})(.*)", line
                    )
                    if match == None:
                        continue
                    ...
                    for c in self.conditions:
                        if (
                            c.match(eventType, message) != None
                            and not line in alreadyPrintedLines
                        ):
