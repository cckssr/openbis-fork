...
class TestCase(object):
    ...
    def runTest(self):
        ...
        try:
            ...
            if success:
                util.printAndFlush(
                    "\...........SUCCESS: %s executed in %s .........."
                    % (self.name, duration)
                )
            else:
                util.printAndFlush(
                    "\............FAILED: %s executed in %s .........."
                    % (self.name, duration)
                )
    ...
    def assertSmaller(self, itemName, expectedUpperLimit, actualValue, verbose=True):
        ...
        if actualValue > expectedUpperLimit:
            ...
            util.printAndFlush(
                "%s actual value <%s> is below the expected upper limit <%s>"
                % (itemName, actualValue, expectedUpperLimit)
            )
        return True
    ...
    def assertEquals(self, itemName, expected, actual, verbose=True):
        ...
        if expected != actual:
            ...
            diff = difflib.ndiff(
                rendered_expected.splitlines(), rendered_actual.splitlines()
            )
            self.fail("%s\n  Differences:\n%s" % (itemName, "\n".join(diff)))
            return False
        elif verbose:
    ...
    def installScriptBasedServer(
        ...
        shutil.copytree(
            "%s/%s" % (self.getTemplatesFolder(), templateName), installPath
        )
        return ScriptBasedServerController(
            self, self.name, installPath, instanceName, startCommand, stopCommand
        )

    def createScriptBasedServerController(
        self, instanceName, startCommand=["./start.sh"], stopCommand=["./stop.sh"]
    ):
        return ScriptBasedServerController(
            self,
            self.name,
            self._getInstallPath(instanceName),
            instanceName,
            startCommand,
            stopCommand,
        )
    ...
    def installDatamover(self, instanceName="datamover"):
        zipFile = self.artifactRepository.getPathToArtifact(
            DATAMOVER_PROJECT, "datamover"
        )
        installPath = self._getInstallPath(instanceName)
    ...
    def createDatamoverController(self, instanceName="datamover"):
        return DatamoverController(
            self, self.name, self._getInstallPath(instanceName), instanceName
        )
    ...
    def installOpenbis(self, instanceName="openbis", technologies=[]):
        ...
        """
        installerPath = self.artifactRepository.getPathToArtifact(
            INSTALLER_PROJECT, "openbis-installer"
        )
        installerFileName = os.path.basename(installerPath).rpartition(".tar")[0]
        util.executeCommand(
            ["tar", "-zxf", installerPath, "-C", self.playgroundFolder],
            "Couldn't untar openBIS installer.",
        )
        consolePropertiesFile = "%s/%s/console.properties" % (
            self.playgroundFolder,
            installerFileName,
        )
        ...
        util.executeCommand(
            "%s/%s/run-console.sh" % (self.playgroundFolder, installerFileName),
            "Couldn't install openBIS",
            consoleInput="admin\nadmin",
        )
        shutil.rmtree("%s/%s" % (self.playgroundFolder, installerFileName))

    def cloneOpenbisInstance(
        self, nameOfInstanceToBeCloned, nameOfNewInstance, dataStoreServerOnly=False
    ):
        """Clones an openBIS instance."""

        oldInstanceInstallPath = "%s/%s" % (
            self.playgroundFolder,
            nameOfInstanceToBeCloned,
        )
        ...
        dssPropsFile = (
            "%s/servers/datastore_server/etc/service.properties"
            % newInstanceInstallPath
        )
        dssProps = util.readProperties(dssPropsFile)
        dssProps["root-dir"] = dssProps["root-dir"].replace(
            nameOfInstanceToBeCloned, nameOfNewInstance
        )
        util.writeProperties(dssPropsFile, dssProps)

    def createOpenbisController(
        self,
        instanceName="openbis",
        port="8443",
        dropDatabases=True,
        databasesToDrop=[],
    ):
    ...
    def installScreeningTestClient(self):
        ...
        zipFile = self.artifactRepository.getPathToArtifact(
            OPENBIS_STANDARD_TECHNOLOGIES_PROJECT, "openBIS-screening-API"
        )
    ...
    def installPybis(self):
        ...
        util.executeCommand(
            ["pip3", "install", "-e", "../api-openbis-python3-pybis/src/python"],
    ...
    def installObis(self):
        ...
        util.executeCommand(
            ["pip3", "install", "-e", "../app-openbis-command-line/src/python"],
...
class _Controller(object):
    def __init__(self, testCase, testName, installPath, instanceName):
        ...
        util.printAndFlush(
            "Controller created for instance '%s'. Installation path: %s"
    ...
    def assertFiles(self, folderPathRelativeToInstallPath, expectedFiles):
        ...
        self.testCase.assertEquals(
            "Files in %s" % relativePath, expectedFiles, sorted(files)
        )
...
class ScriptBasedServerController(_Controller):
    def __init__(
        self, testCase, testName, installPath, instanceName, startCommand, stopCommand
    ):
        super(ScriptBasedServerController, self).__init__(
            testCase, testName, installPath, instanceName
        )
    ...
    def start(self):
        ...
        util.executeCommand(
            self.startCommand,
            "Couldn't start server '%s'" % self.instanceName,
    ...
    def stop(self):
        ...
        util.executeCommand(
            self.stopCommand,
            "Couldn't stop server '%s'" % self.instanceName,
            workingDir=self.installPath,
        )
...
class DatamoverController(_Controller):
    def __init__(self, testCase, testName, installPath, instanceName):
        super(DatamoverController, self).__init__(
            testCase, testName, installPath, instanceName
        )
        ...
        dataCompletedScript = "%s/%s/data-completed.sh" % (
            testCase.getTemplatesFolder(),
            instanceName,
        )
        if os.path.exists(dataCompletedScript):
            self.serviceProperties["data-completed-script"] = (
                "../../../../%s" % dataCompletedScript
            )
    ...
    def start(self):
        ...
        output = util.executeCommand(
            ["%s/datamover.sh" % self.installPath, "start"], suppressStdOut=True
        )
        joinedOutput = "\n".join(output)
        if "FAILED" in joinedOutput:
            util.printAndFlush(
                "Start up of datamover %s failed:\n%s"
                % (self.instanceName, joinedOutput)
            )
    ...
    def stop(self):
        ...
        self.testCase._removeFromRunningInstances(self)
        util.executeCommand(
            ["%s/datamover.sh" % self.installPath, "stop"],
    ...
    def drop(self, testDataSetName):
        ...
        util.copyFromTo(
            "%s/%s" % (TEST_DATA, self.testName),
            "%s/data/incoming" % self.installPath,
            testDataSetName,
        )
...
class DataSet(object):
    ...
    def __str__(self):
        ...
        return "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s" % (
            ...
            self.location,
            self.status,
            self.presentInArchive,
            parents,
            children,
            self.experimentCode,
            self.producer,
            self.productionTimeStamp,
        )
...
class OpenbisController(_Controller):
    ...
    def __init__(
        ...
        """
        super(OpenbisController, self).__init__(
            testCase, testName, installPath, instanceName
        )
        ...
        self.asServicePropertiesFile = (
            "%s/servers/openBIS-server/jetty/etc/service.properties" % installPath
        )
        ...
        if os.path.exists(self.asServicePropertiesFile):
            ...
            self.asProperties["api.v3.transaction.interactive-session-key"] = (
                "test-interactive-session-key-for-pybis"
            )
            # self.asProperties['server-public-information.afs-server.url'] = 'true'
            self.asProperties[
                "api.v3.transaction.participant.application-server.url"
            ] = "jenkins:8443/openbis"

            self.asPropertiesModified = True
        self.dssServicePropertiesFile = (
            "%s/servers/datastore_server/etc/service.properties" % installPath
        )
        ...
        self.passwdScript = (
            "%s/servers/openBIS-server/jetty/bin/passwd.sh" % installPath
        )
        if port != "8443":
            self.sslIniFile = (
                "%s/servers/openBIS-server/jetty/start.d/ssl.ini" % installPath
            )
    ...
    def assertNumberOfDataSets(self, expectedNumberOfDataSets, dataSets):
        ...
        for dataSet in dataSets:
            if (
                dataSet.dataStore != self.getDataStoreServerCode()
                or dataSet.location == ""
            ):
                continue
            count += 1
    ...
    def getDataSets(self):
        ...
        relationships = self.queryDatabase(
            "openbis",
    ...
    def allUp(self):
        ...
        if not util.isAlive(
            "%s/servers/openBIS-server/jetty/openbis.pid" % self.installPath,
            "openBIS.keystore",
        ):
    ...
    def dssUp(self):
        ...
        if not util.isAlive(
            "%s/servers/datastore_server/datastore_server.pid" % self.installPath,
            "openBIS.keystore",
        ):
    ...
    def dropAndWait(
        ...
        self.waitUntilDataSetRegistrationFinished(
            numberOfDataSets=numberOfDataSets, timeOutInMinutes=timeOutInMinutes
        )
    ...
    def waitUntilDataSetRegistrationFinished(
        self, numberOfDataSets=1, timeOutInMinutes=DEFAULT_TIME_OUT_IN_MINUTES
    ):
        ...
        while numberOfRegisteredDataSets < numberOfDataSets:
            condition1 = util.RegexCondition(
                "Post registration of (\\d*). of \\1 data sets"
            )
            condition2 = util.RegexCondition(
                "Paths inside data set .* successfully added to database"
            )
            elements = monitor.waitUntilEvent(
                util.ConditionSequence([condition1, condition2])
            )
            numberOfRegisteredDataSets += int(elements[0][0])
            util.printAndFlush(
                "%d of %d data sets registered"
                % (numberOfRegisteredDataSets, numberOfDataSets)
            )

    def waitUntilDataSetRegistrationFailed(
        self, timeOutInMinutes=DEFAULT_TIME_OUT_IN_MINUTES
    ):
        """Waits until data set registration failed."""
        self.waitUntilConditionMatched(
            util.EventTypeCondition("ERROR"), timeOutInMinutes
        )
        util.printAndFlush("Data set registration failed as expected.")

    def waitUntilConditionMatched(
        self, condition, timeOutInMinutes=DEFAULT_TIME_OUT_IN_MINUTES
    ):
        """
    ...
    def createLogMonior(self, timeOutInMinutes=DEFAULT_TIME_OUT_IN_MINUTES):
        logFilePath = (
            "%s/servers/datastore_server/log/datastore_server.log" % self.installPath
        )
        return util.LogMonitor(
            "%s.DSS" % self.instanceName, logFilePath, timeOutInMinutes
        )
    ...
    def assertFeatureVectorLabel(self, featureCode, expectedFeatureLabel):
        ...
        self.testCase.assertEquals(
            "label of feature %s" % featureCode, [[expectedFeatureLabel]], data
        )
    ...
    def addUser(self, name, password):
        util.executeCommand(
            [self.passwdScript, "add", name, "-p", password],
    ...
    def _setUpStore(self):
        ...
        if os.path.isdir(templateStore):
            ...
            util.printAndFlush(
                "Set up initial data store by copying content of %s to %s"
    ...
    def _setUpFileServer(self):
        templateFileServer = "%s/file-servers/%s" % (
        ...
        if os.path.isdir(templateFileServer):
            ...
            util.printAndFlush(
                "Set up initial file server by copying content of %s to %s"
