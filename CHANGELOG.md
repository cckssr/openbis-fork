...
- Bugfix: Export ignores Space Objects and Project Objects (BIS-2031)
- Bugfix: Some folder objects are not show in ELN menu in some Projects/Spaces (BIS-2030)
- Bugfix: An error when trying to freeze an entity (BIS-1997)
- Bugfix: Exports to zip fails (BIS-1994)
- Bugfix: $properties not imported, even if they are already present in openBIS 20.10.x (BIS-1991)
- Bugfix: Some new features missing from the ELN Settings Menu with older settings saved (BIS-1933)
...
- Bugfix: When a SPACE or PROJECT is deleted, message says it will be moved to the trashcan, which is not correct (BIS-968)
...
- Bugfix: Archiving / Unarchiving helper was working properly after ELN UI Update (BIS-2074)
- Bugfix: Missing features in request and order form after ELN UI Update (BIS-2076)
- Bugfix: ELN-LIMS dropbox complains about "NAME" property non existing after ELN UI Update (BIS-2034)
...
- Bugfix: Navigation menu not showing correctly entries linked with parent/child relatioships (BIS-1937)
- Bugfix: Views don't load properly sometimes when using Back-Forth Browser buttons (BIS-1863)
...
- Improvement: What are standard and default ELN types re-defined (BIS-1872)
...
- Improvement: Adding Zenodo Configuration to the documentation (BIS-1783)
- Improvement: XLS Import - More Efficient XLS Export for Samples (BIS-1721)
- Improvement: XLS Import - Don't use the same property label twice as a column name on import templates (BIS-1701)
...
- Improvement: object/collection forms in inventory do not have "+Dataset" button
- Improvement: Navigation menu not refreshed when switching between Lab notebook, Inventory, Tools when using Switchaai authentication (BIS-1992)
