...
class LockManager<O, E>
{
    ...
    private LockMapper<O, E> lockMapper;
    ...
    LockManager(LockMapper<O, E> lockMapper, HierarchicalLockFinder<O, E> hierarchicalLockFinder)
    {
    ...
    {
        return unmapLocks(hierarchicalLockFinder.getHierarchicallyExclusiveLocks());
    }

    Map<E, Lock<O, E>> getExclusiveLocks()
    {
        return unmapLocks(exclusiveLocks);
    }

    Map<E, Set<Lock<O, E>>> getSharedLocks()
    {
        return sharedLocks.entrySet().stream().map(entry -> new AbstractMap.SimpleEntry<>(entry.getKey(), unmapLocks(entry.getValue())))
    ...
    synchronized boolean add(List<Lock<O, E>> locksList)
    {
        List<Lock<O, E>> locks = mapLocks(locksList);
    ...
    synchronized boolean remove(List<Lock<O, E>> locksList)
    {
        List<Lock<O, E>> locks = mapLocks(locksList);
    ...
    private List<Lock<O, E>> mapLocks(List<Lock<O, E>> locksList)
    {
        return locksList != null ?
                locksList.stream().map(lock -> new MappedLock<O, E>(lock, lockMapper.mapLock(lock))).collect(Collectors.toList()) :
                Collections.emptyList();
    }

    private Set<Lock<O, E>> unmapLocks(Set<Lock<O, E>> locksSet)
    {
        return locksSet != null ?
                locksSet.stream().map(lock -> ((MappedLock<O, E>) lock).getOriginalLock()).collect(Collectors.toSet()) :
                Collections.emptySet();
    }

    private Map<E, Lock<O, E>> unmapLocks(Map<E, Lock<O, E>> locksMap)
    {
        return locksMap != null ? locksMap.values().stream().map(lock -> ((MappedLock<O, E>) lock).getOriginalLock())
                .collect(Collectors.toMap(Lock::getResource, lock -> lock)) : Collections.emptyMap();
    }

    private static class MappedLock<O, E> extends Lock<O, E>
    {

        private final Lock<O, E> originalLock;

        public MappedLock(Lock<O, E> originalLock, Lock<O, E> mappedLock)
        {
            super(mappedLock.getOwner(), mappedLock.getResource(), mappedLock.getType());
            this.originalLock = originalLock;
        }

        public Lock<O, E> getOriginalLock()
        {
            return originalLock;
        }
    }
