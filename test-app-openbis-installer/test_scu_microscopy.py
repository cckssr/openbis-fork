...
class TestCase(systemtest.testcase.TestCase):
    ...
    def getTestDataFolder(self):
        ...
        if os.path.exists(testDataFolder):
            ...
            self.fail(
                "Test data folder missing: %s - current path: %s" % (testDataFolder, os.getcwd()))
    ...
    def dropTestExample(self, openbisController, testDataFolder, exampleName):
        ...
        try:
            openbisController.waitUntilDataSetRegistrationFinished(
                expectations.numberOfPhysicalDataSets,
                timeOutInMinutes=expectations.registrationTimeout)
            self.waitUntilThumbnailGenerationTaskFinished(openbisController,
                                                          expectations.numberOfThumbnailDataSets,
                                                          timeOutInMinutes=expectations.registrationTimeout)
            logInfo = self.getLogInfo(openbisController, exampleName)
            print("Number of experiments created equals " + str(
                expectations.numberOfExperiments) + " " + str(logInfo.numberOfExperiments))
            print(
                "Number of samples created equals " + str(expectations.numberOfSamples) + " " + str(
                    logInfo.numberOfSamples))
            print("Number of data sets created equals " + str(
                expectations.numberOfDataSets) + " " + str(logInfo.numberOfDataSets))
            print("Thumbnail generation time smaller " + str(
                expectations.thumbnailGenerationTime) + " " + str(logInfo.thumbnailTime))
            print("Registration time smaller " + str(expectations.registrationTime) + " " + str(
                logInfo.jobTime))
        except Exception:
            logInfo = self.getLogInfo(openbisController, exampleName)
            print("Number of experiments created equals " + str(
        ...
        self.assertEquals("Number of experiments created", expectations.numberOfExperiments,
                          logInfo.numberOfExperiments)
        self.assertEquals("Number of samples created", expectations.numberOfSamples,
                          logInfo.numberOfSamples)
        self.assertEquals("Number of data sets created", expectations.numberOfDataSets,
                          logInfo.numberOfDataSets)
        self.assertSmaller("Thumbnail generation time", expectations.thumbnailGenerationTime,
                           logInfo.thumbnailTime)
        self.assertSmaller("Registration time", expectations.registrationTime, logInfo.jobTime)

    def waitUntilThumbnailGenerationTaskFinished(self, openbisController,
                                                 numberOfExpectedThumbnailDataSets,
                                                 timeOutInMinutes):
        ...
        monitor.waitUntilEvent(util.RegexCondition(
            "%s thumbnail data sets have been created" % numberOfExpectedThumbnailDataSets))
