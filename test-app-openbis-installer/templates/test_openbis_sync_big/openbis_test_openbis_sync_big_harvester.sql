...
-- Remove property type assignments for MATERIAL-typed property types
-- These cause DataType.MATERIAL enum lookup failures in PropertyTypeTranslator
DELETE FROM sample_type_property_types WHERE prty_id IN (
    SELECT id FROM property_types WHERE daty_id = (SELECT id FROM data_types WHERE code = 'MATERIAL')
);
DELETE FROM experiment_type_property_types WHERE prty_id IN (
    SELECT id FROM property_types WHERE daty_id = (SELECT id FROM data_types WHERE code = 'MATERIAL')
);
