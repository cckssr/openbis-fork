...
public class DataSetFileOperationsManager extends AbstractDataSetFileOperationsManager implements IDataSetFileOperationsManager
{
            ...
            ISshCommandExecutorFactory sshCommandExecutorFactory)
    {
        long timeoutInSeconds =
                PropertyUtils.getLong(properties, AbstractDataSetFileOperationsManager.TIMEOUT_KEY, AbstractDataSetFileOperationsManager.DEFAULT_TIMEOUT_SECONDS);
        long timeoutInMillis = timeoutInSeconds * DateUtils.MILLIS_PER_SECOND;

        String hostFile = PropertyUtils.getMandatoryProperty(properties, AbstractDataSetFileOperationsManager.DESTINATION_KEY);
        ...
        return delete(dataset, new IDeleteAction()
            {
                ...
                {
                    File deletedFolder =
                            new File(archiveDestination.getDestination(), AbstractDataSetFileOperationsManager.FOLDER_OF_AS_DELETED_MARKED_DATA_SETS);
    ...
    private void checkDestinationExists(File destinationFolder)
    {
        ...
        if (destinationExists.isSuccess() == false)
        {
            ...
            throw new ExceptionWithStatus(Status.createError(AbstractDataSetFileOperationsManager.DESTINATION_DOES_NOT_EXIST));
        }
    }
    ...
    private BooleanStatus destinationExists(File destinationFolder)
    {
        ...
        if (destinationExists.isError())
        {
            ...
            throw new ExceptionWithStatus(Status.createError(AbstractDataSetFileOperationsManager.CHECK_EXISTENCE_FAILED));
        }
