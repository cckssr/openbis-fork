...
public class AttributeValidator<E extends IAttribute>
{
        ...
        for (IAttribute attribute : attributes)
        {
            if (attribute.isMandatory() && !headers.containsKey(attribute.getHeaderName()))
            {
                throw new UserFailureException(
                        "Header '" + attribute.getHeaderName() + "' is missing.");
            }
        }
    }

    public void validateHeaders(E[] attributes, PropertyTypeSearcher propertyTypeSearcher,
            Map<String, Integer> headers)
    {
        for (String header : headers.keySet())
        {
            ...
            throw new UserFailureException(
                    "Header '" + header + "' is neither an attribute, property code or property label.");
        }
    ...
    public static void validateHeader(String valid, Map<String, Integer> headers)
    {
        for (String header : headers.keySet())
        {
            ...
            {
                throw new UserFailureException(
                        "Header '" + header + "' is not a valid header. Check for typos, the only valid header is: '" + valid + "'.");
            }
        }
    }
