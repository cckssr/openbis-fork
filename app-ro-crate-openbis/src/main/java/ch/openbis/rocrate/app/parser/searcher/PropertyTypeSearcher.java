 ...
 */
package ch.openbis.rocrate.app.parser.searcher;
...
public class PropertyTypeSearcher
{
    ...
    public static final SimpleDateFormat timestampFormatter =
            new SimpleDateFormat(new LongDateFormat().getFormat());

    public static final SimpleDateFormat dateFormatter =
            new SimpleDateFormat(new ShortDateFormat().getFormat());
    ...
    public PropertyTypeSearcher(List<PropertyAssignment> assignment)
    {
        ...
        for (PropertyAssignment propertyAssignment : assignment)
        {
            ...
            if (label2PropertyType.containsKey(
                    propertyType.getLabel()) == false) // If a label already exists, the next one is ignored
            {
    ...
    private static Serializable getPropertyValueInternal(PropertyType propertyType, String value)
    {
        ...
        } else if (propertyType.getDataType() == DataType.TIMESTAMP)
        { // Converts native excel timestamps
            if (value != null && isDouble(value))
            {
                value = timestampFormatter.format(DateUtil.getJavaDate(Double.parseDouble(value)));
            }
        } else if (propertyType.getDataType() == DataType.DATE)
        { // Converts native excel dates
            if (value != null && isDouble(value))
            {
