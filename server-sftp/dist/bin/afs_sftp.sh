#!/bin/bash
#
# Control script for openBIS AFS SFTP on Unix / Linux systems
# -------------------------------------------------------------------------
...
{
  if [ $UID -eq 0 ]; then
    echo "openBIS AFS SFTP cannot run as user 'root'." > /dev/stderr
    exit 1
  fi
}
...
printStatus()
{
  if [ -f $PID_FILE ]; then
    ...
    isPIDRunning $PID
    if [ $? -eq 0 ]; then
      echo "AFS SFTP is running (pid $PID)"
      return 0
    else
      echo "AFS SFTP is dead (stale pid $PID)"
      return 1
    fi
  else
    echo "AFS SFTP is not running."
    return 2
  fi
}
...
PID_FILE="$AFS_SFTP_HOME/afs_sftp.pid"
LIB_FOLDER=./lib
CONF_FILE=./etc/afs_sftp.conf
SERVICE_PROPERTIES_FILE=./etc/service.properties
LOG_FOLDER=./log
LOG_FILE=$LOG_FOLDER/afs_sftp.log
#SUCCESS_MSG="=== Server ready ==="
MAX_LOOPS=20

STARTED_MARKER="$AFS_SFTP_HOME/SERVER_STARTED"
TIMEOUT=120
...
JAVA_OPTIONS="${JAVA_OPTS} ${JAVA_MEM_OPTS} -Dio.netty.tryReflectionSetAccessible=true -Djava.awt.headless=true --add-opens=java.base/jdk.internal.misc=ALL-UNNAMED --add-opens=java.base/java.nio=ALL-UNNAMED -classpath $JAVA_CLASS_PATH ch.ethz.sis.afssftp.startup.Main $SERVICE_PROPERTIES_FILE"
...
case "$command" in
  start)
    ...
    if [ $EXIT_STATUS -eq 0 ]; then
      echo "Cannot start AFS SFTP: already running."
      exit 100
    fi

    echo -n "Starting AFS SFTP "
