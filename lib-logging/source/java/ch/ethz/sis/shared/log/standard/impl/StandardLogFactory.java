...
public class StandardLogFactory implements LogFactory {
    ...
    public <T> Logger getLogger(Class<T> clazz) {
        return StandardLogger.getStandardLogger(clazz);
    }

    @Override public Logger getLogger(String name)
    {
        return StandardLogger.getStandardLogger(name);
    }
    ...
    public void configure(String pathToConfigurationFile) {
        if (pathToConfigurationFile != null && !pathToConfigurationFile.isBlank()) {
            ...
            LoggerDiagnostics.info("[StandardLogFactory] No configuration file provided, skipping initialization!");
        }
    }
}