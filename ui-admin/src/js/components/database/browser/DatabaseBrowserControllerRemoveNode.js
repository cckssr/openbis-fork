...
export default class DatabaseBrowserControllerRemoveNode {
  ...
  async doRemoveNode(selectedObject) {
    ...
    try {
      ...
      await AppController.getInstance().objectDelete(pages.DATABASE, type, id)
    } catch (error) {
  ...
  async _prepareRemoveOperations(type, id, reason) {
    console.log('DatabaseBrowserControllerRemoveNode._prepareRemoveOperations', {type, id, reason});
    /* if (
  ...
  async _prepareRemoveEntityTypeOperations(type, id, reason) {
    ...
    } else if (type === objectType.DATA_SET) {
      const options = new openbis.DataSetTypeDeletionOptions()
      options.setReason(reason)
      operations.push(
        new openbis.DeleteDataSetTypesOperation(
