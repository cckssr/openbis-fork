...
export default class EntityTypeFormControllerLoad extends PageControllerLoad {
  ...
  _createProperty(id, loadedAssignment, loadedAssignments, entityType, propertySemanticAnnotations, assignmentSemanticAnnotations) {
    ...
    const entityTypeInternal = _.get(entityType, 'managedInternally', false)
    ...
    return {
      ...
      label: FormUtil.createField({
        ...
        enabled:
          !propertyTypeInternal || isSystemUser
      }),
      description: FormUtil.createField({
      ...
      metadata: FormUtil.createField({
        value: metadata,
        enabled: (!entityTypeInternal && !assignmentInternal && !propertyTypeInternal) || isSystemUser
      }),
      ...
      semanticAnnotations: FormUtil.createField({
        ...
        enabled: !assignmentInternal || isSystemUser
      }),
...
class ObjectTypeStrategy {
  setTypeAttributes(object, loadedType) {
    const internal = _.get(loadedType, 'managedInternally', false) || AppController.getInstance().isSystemUser()
