...
public interface SampleQuery extends ObjectQuery
{
    ...
    @Select(sql =
            "select st.id, st.code, st.description, st.is_listable as listable, st.is_subcode_unique as subcodeUnique, st.is_auto_generated_code as autoGeneratedCode, "
                    ...
                    + "st.modification_timestamp as modificationDate, st.meta_data as metaData, st.is_managed_internally as managedInternally from sample_types st where st.id = any(?{1})", parameterBindings = {
            LongSetMapper.class }, fetchSize = FETCH_SIZE)
