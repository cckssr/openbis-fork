...
class SampleExpectations extends Expectations
{
    ...
    public SampleExpectations(final IApplicationServerApi api, final boolean exportReferred)
    {
        ...
        will(new GetSamplesAction(null));

        allowing(api).getSampleTypes(with(XLSExportTest.SESSION_TOKEN), with(new CollectionMatcher<>(
                        Collections.singletonList(new EntityTypePermId("STORAGE", EntityKind.SAMPLE)))),
        ...
        will(new CustomAction("getting sample types")
        {
            ...
            public Object invoke(final Invocation invocation)
            {
                ...
                sampleType.setDescription("Storage");
                ...
                sampleType.setGeneratedCodePrefix("");
                ...
                sampleType.setModificationDate(calendar.getTime());

                return Map.of(sampleType.getPermId(), sampleType);
            }

        });

        allowing(api).getSampleTypes(with(XLSExportTest.SESSION_TOKEN), with(new CollectionMatcher<>(
                        Collections.singletonList(new EntityTypePermId("DEFAULT", EntityKind.SAMPLE)))),
                with(any(SampleTypeFetchOptions.class)));
        will(new CustomAction("getting sample types")
        {
            ...
            public Object invoke(final Invocation invocation) throws Throwable
            {
                ...
                defaultSampleType.setManagedInternally(false);
                defaultSampleType.setDescription("Default");
                defaultSampleType.setPermId(new EntityTypePermId("DEFAULT", EntityKind.SAMPLE));
                defaultSampleType.setFetchOptions(fetchOptions);
                defaultSampleType.setPropertyAssignments(List.of(namePropertyAssignment));
                defaultSampleType.setTypeGroupAssignments(List.of());
                defaultSampleType.setAutoGeneratedCode(false);
                defaultSampleType.setGeneratedCodePrefix("");
                defaultSampleType.setSubcodeUnique(false);

                final Calendar calendar = Calendar.getInstance();
                calendar.set(2023, Calendar.MARCH, 11, 17, 23, 44);
                defaultSampleType.setModificationDate(calendar.getTime());

                return Map.of(defaultSampleType.getPermId(), defaultSampleType);
            }

        });
    }

    private static PropertyAssignment getNamePropertyAssignment()
    {
        final PropertyAssignmentFetchOptions propertyAssignmentFetchOptions = getPropertyAssignmentFetchOptions();
        ...
        propertyAssignment.setPlugin(null);

        return propertyAssignment;
    }

    private static PropertyAssignmentFetchOptions getPropertyAssignmentFetchOptions()
    {
        final PropertyAssignmentFetchOptions fetchOptions = new PropertyAssignmentFetchOptions();
        fetchOptions.withPropertyType().withVocabulary();
        fetchOptions.withPlugin().withScript();
        return fetchOptions;
    }

    private static PropertyAssignment getBoxesCountPropertyAssignment()
    {
        final PropertyAssignmentFetchOptions propertyAssignmentFetchOptions = getPropertyAssignmentFetchOptions();
        final PropertyTypeFetchOptions propertyTypeFetchOptions = propertyAssignmentFetchOptions.withPropertyType();
    ...
    private static class GetSamplesAction extends CustomAction
    {

        /** If only one sample should be returned, this should be set. */
        private final Integer sampleIndex;

        public GetSamplesAction(final Integer sampleIndex)
        {
            super("getting samples");
        ...
        public Object invoke(final Invocation invocation)
        {
            ...
            {
                final Sample sample = samples[sampleIndex];
                return Map.of(sample.getPermId(), sample);
            }
        }
