...
public class CollectionHelper extends BasicImportHelper
{
    ...
    public void importBlock(List<List<String>> page, int pageIndex, int start, int end)
    {
        ...
        try
        {
            ...
            AttributeValidator.validateHeader(CollectionTypeHelper.EXPERIMENT_TYPE_FIELD, header);
            lineIndex++;

            experimentType = new EntityTypePermId(
                    getValueByColumnName(header, page.get(lineIndex),
                            CollectionTypeHelper.EXPERIMENT_TYPE_FIELD),
                    EntityKind.EXPERIMENT);
            if (experimentType.getPermId() == null || experimentType.getPermId().isEmpty())
            {
                throw new UserFailureException(
                        "Mandatory field is missing or empty: " + CollectionTypeHelper.EXPERIMENT_TYPE_FIELD);
            }
        ...
        for (String key : header.keySet())
        {
            if (!attributeValidator.isHeader(key))
            {
                ...
                exerpiment.setProperty(propertyType.getCode(),
                        PropertyTypeSearcher.getPropertyValue(propertyType, value));
            }
        }
