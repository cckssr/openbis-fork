...
public class Copier implements Serializable
{
            ...
            IImmutableCopierFactory immutableCopierFactory)
    {
        ...
        rsyncPasswordFile = properties.getProperty(DataSetCopierConst.RSYNC_PASSWORD_FILE_KEY);
        rsyncExecutable = getExecutable(properties, DataSetCopierConst.RSYNC_EXEC);
        sshExecutable = getExecutable(properties, DataSetCopierConst.SSH_EXEC);
        hostFile = PropertyUtils.getMandatoryProperty(properties, DataSetCopierConst.DESTINATION_KEY);
        renameToDataSetCode = PropertyUtils.getBoolean(properties, DataSetCopierConst.RENAME_TO_DATASET_CODE_KEY, false);
        hardLinkCopy = PropertyUtils.getBoolean(properties, DataSetCopierConst.HARD_LINK_COPY_KEY, false);
        if (hardLinkCopy)
        {
            ...
            lnExecutable = getExecutable(properties, DataSetCopierConst.LN_EXEC);
        }
    }
    ...
    public Status handle(File originalData, String dataSetCode,
            Map<String, String> parameterBindings)
    {
        ...
        IPathCopier copier =
                pathCopierFactory.create(rsyncExecutable, sshExecutable,
                        DataSetCopierConst.SSH_TIMEOUT_MILLIS, RSyncConfig.getInstance().getAdditionalCommandLineOptions());
        copier.check();
        if (host != null)
        {
            FileUtilities.checkPathCopier(copier, host, null, rsyncModule, rsyncPasswordFile,
                    DataSetCopierConst.SSH_TIMEOUT_MILLIS);
        }
        ...
        if (destinationExists.isSuccess())
        {
            ...
            return Status.createError(DataSetCopierConst.ALREADY_EXIST_MSG);
        } else if (destinationExists.isError())
        {
            ...
            return Status.createError(DataSetCopierConst.COPYING_FAILED_MSG);
        }
        Status status;
        if (hardLinkCopy)
        {
            ...
            } else
            {
                ...
                ProcessResult result =
                        sshCommandExecutor.executeCommandRemotely("mv " + destinationFile.getPath()
                                + " " + newFilePath, DataSetCopierConst.SSH_TIMEOUT_MILLIS);
