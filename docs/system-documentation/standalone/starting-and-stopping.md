...
```console
prompt> <installation folder>/bin/as-service.sh start
```
...
```console
prompt> <installation folder>/bin/as-service.sh stop
```

To only stop the data store server:
```console
prompt> <installation folder>/bin/dss-service.sh stop
```

To stop both the data store server and then the applicaiton server:
```console
prompt> <installation folder>/bin/openbis.sh stop
```
