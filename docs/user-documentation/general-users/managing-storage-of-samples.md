...
openBIS provides a graphical overview of lab storages, to track physical storage of materials and samples. 
...
Storage information of samples can be added in two ways:
...
form. Click the **+ Storage Position** above the table, as shown
below:

![image info](img/201012-object-form-add-new-storage-position.png)
...
![image info](img/201012-object-form-storage-position.png)
...
With the XLS batch registration, samples and their storage positions
can be registered in one transaction using the XLS template file, as
explained in [Batch register entries in a
Collection](./inventory-of-materials-and-methods.md#batch-register-entries-in-a-collection).
...
1.  Go to the **Object Browser** under **Utilities** in the main menu, under the **Tools** tab (see picture).

![image info](img/201012-object-browser-dropdown-storage-position.png)


2.  Use the table **Filter** to select the storage positions you want to update (see [Tables](./additional-functionalities.md#tables)).
3.  Export the table (see [Tables](./additional-functionalities.md#tables)).
4.  Edit the file to make the changes needed (e.g. change the name of a box, change the storage, change a box position, change box size etc).
5.  Select **XLS Batch Update Objects** from the **More..** dropdown.

![image info](img/201012-object-browser-batch-update.png)

6. Import the file you modified before and update the storage
positions.
...
3.  Use the “**–**” button in the **Storage Position** table, as shown in the picture below.
4.  **Save** the sample.

![image info](img/201012-object-form-remove-storage-position.png)
...
from the trashcan (see [Trashcan](./additional-functionalities.md#trashcan)).
...
1.  Go to the **Object Browser** under **Utilities** in the main menu, under the **Tools** tab.
...
delete (see [Tables](./additional-functionalities.md#tables)).
...
[Trashcan) ](./additional-functionalities.md#trashcan).

 

![image info](img/201012-object-browser-delete-storage-positions.png)

 

## Overview of lab storages
  
The **Storage Manager**, under **Utilities**, in the **Tools** tab, provides an overview of
...
![image info](img/201012-storage-manager-overview.png)
...
3.  Drag and drop the box or sample to move from **Storage A** to the desired position in **Storage B**. Please note that the move
