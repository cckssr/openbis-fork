...
/**
 * Immutable class with path informations about a file and its descendants.
 *
...
public final class PathInfo
{
    ...
    public static PathInfo createPathInfo(File node, boolean computeChecksum, String checksumType)
    {
        if (node.exists() == false)
        {
            throw new IllegalArgumentException("File does not exist: " + node.getPath());
        }
        PathInfo pathInfo = new PathInfo();
        pathInfo.fileName = node.getName();
        pathInfo.lastModifiedDate = new Date(node.lastModified());
        ...
        {
            pathInfo.sizeInBytes = node.length();
            setChecksum(pathInfo, node, computeChecksum, checksumType);
        }
        return pathInfo;
    }

    private static void setChecksum(PathInfo pathInfo, File node,
            boolean computeChecksum, String checksumType)
    {
        ...
        try
        {
            setChecksum(pathInfo, new BufferedInputStream(new FileInputStream(node)), computeChecksum, checksumType);
        } catch (IOException e)
        {
    ...
    private static List<PathInfo> createPathInfos(File node, boolean computeChecksum, String checksumType)
    {
        if (node.isDirectory() == false)
        {
            throw new IllegalArgumentException("Not a folder: " + node.getPath());
        }
